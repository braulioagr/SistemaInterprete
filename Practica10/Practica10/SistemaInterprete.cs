...
namespace Practica10
{
    public partial class SistemaInterprete : Form
    {
        ...
        private void seleccionaArquitectura()
        {
            this.ensamblador = null;
            this.seleccion = new Seleccion();
            this.extendida = this.seleccion.ShowDialog().Equals(DialogResult.OK);
            if (this.extendida)
            {
                this.openFileProgram.Filter = "SICExtendida (*.sx)|*.sx";
            }
            else
            {
                this.openFileProgram.Filter = "SICEstandar (*.s)|*.s";
            }
